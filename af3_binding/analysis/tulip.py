...
for i in range(1,len(all_results)):
    if int(all_results.iloc[i,2]) == 1:
        positive_predictions.append(1/(1+np.exp(-float(all_results.iloc[i, 1]))))
    else:
        negative_predictions.append(1/(1+np.exp(-float(all_results.iloc[i, 1]))))
...
plt.savefig('photo/predict_distribution_tulip.jpg', dpi=300)
...
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('photo/predict_distribution_tulip_kde.jpg', dpi=300)