...
def evaluate(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="评估进度", ncols=100):
            ...
            # print(scores,torch.sigmoid(scores))
            if count in target:
                model.moe_mhc.plot_moe(str(count)+'10.27')
                print(count,torch.sigmoid(scores))
                print(batch['cdr3a_tokens'])
    ...
    plt.tight_layout()
    # plt.savefig('/home/xycui/project/af3_binding/results/seq_plus_structure_9.23/ranunseen8.png')
    plt.close()
...
def visualize_head(att_map,count = 0):
    plt.clf()
    ax = plt.gca()
    # Plot the heatmap
    ...
    plt.savefig('attn_map/attn_map'+str(count)+'.png')

def visualize_heads(att_map, cols, count = 0):
    ...
    # 找到所有注意力图的最小值和最大值，用于统一颜色映射
    vmin = min(att_map.min(), att_map.mean(axis=0).min())
    vmax = max(att_map.max(), att_map.mean(axis=0).max())
    
    # 绘制每个注意力头
    for i in range(n_heads):
        row = i // cols
        col = i % cols
        im = axes[row, col].imshow(att_map[i], cmap='viridis', aspect='auto', vmin=vmin, vmax=vmax)
        axes[row, col].set_title(f'Head {i}')
        axes[row, col].axis('off')
    
    # 绘制平均注意力头
    average_att_map = att_map.mean(axis=0)
    last_row = (n_heads) // cols
    last_col = n_heads % cols
    im = axes[last_row, last_col].imshow(average_att_map, cmap='viridis', aspect='auto', vmin=vmin, vmax=vmax)
    axes[last_row, last_col].set_title('Head Average')
    axes[last_row, last_col].axis('off')
    
    # 隐藏多余的子图
    total_plots = n_heads + 1
    ...
    plt.savefig('attn_map/seq_map'+str(count)+'.png')

def visualize_heads_compact(att_map,count = 0):
    """
    紧凑版可视化，每个样本的所有头显示在一行中
    """
    batch_size, num_heads, seq_len, _ = att_map.shape
    
    # 每行显示: num_heads个头 + 1个平均头
    cols = num_heads + 1
    rows = batch_size
    ...
    for batch_idx in range(batch_size):
        ...
        for head_idx in range(num_heads):
            im = axes[batch_idx, head_idx].imshow(
                att_map[batch_idx, head_idx], 
                cmap='viridis', aspect='auto', 
                vmin=vmin, vmax=vmax
            )
            axes[batch_idx, head_idx].set_title(f'H{head_idx}')
            axes[batch_idx, head_idx].axis('off')
        
        # 绘制当前样本的平均注意力头
        average_att_map = att_map[batch_idx].mean(axis=0)
        im = axes[batch_idx, num_heads].imshow(
            average_att_map, 
            cmap='viridis', aspect='auto', 
            vmin=vmin, vmax=vmax
        )
        axes[batch_idx, num_heads].set_title('Avg')
        axes[batch_idx, num_heads].axis('off')
        
        # 在左侧添加样本标签
        axes[batch_idx, 0].set_ylabel(f'Sample {batch_idx}', rotation=90, size='large')
    
    # 添加共享颜色条

    plt.subplots_adjust(right=0.85)  # 右侧留出15%的空间给颜色条
    
    # 添加颜色条，放在预留的空间中
    # cbar_ax = fig.add_axes([0.87, 0.15, 0.02, 0.7])  #
    fig.colorbar(im, ax=axes.ravel().tolist(), shrink=0.6, location='right', pad=0.02)

    plt.savefig('attn_map/'+str(count)+'.png')
...
def main():
    ...
    model_name = "seq_plus_structure_10.27"     # 模型名称
    epoch_name = 'epoch_9'
    # model_name = "seq_pus_structure_6.10_WBCE"
    # 配置路径
    test_csv = "/home/xycui/project/af3_binding/dataset/test_immrep23_unseen.csv"
    test_dir = "/home/xycui/project/af3_binding/dataset/test_immrep23_unseen"
    model_weights = "/home/xycui/project/af3_binding/results/"+ model_name +"/model_weights/"+epoch_name+".pt"
    checkpoint_dir = "/home/xycui/project/af3_binding/results/"+ model_name 
    config = load_config_from_checkpoint_dir(checkpoint_dir)
    ...
    attn_maps = cache['AttentionPairBias.forward']
    print(len(attn_maps))
    print(attn_maps[0].shape)

    # visualize_heads(attn_maps[0:4][0], cols=5)

    for i in random.sample(range(int(len(attn_maps)/4)), 3):
        count = i
        # visualize_heads_compact(np.array(attn_maps[count*4+0:count*4+4]).squeeze(1),count)

    important = attn_maps[2::4]
    # print(important.shape) 


    print(list(cache.keys()))
    attn_maps = cache['MOE_MHC.forward']
    print(len(attn_maps))
    print(attn_maps[0].shape)
    for i in random.sample(range(int(len(attn_maps))), 3):
        count = i
        # visualize_heads(np.array(attn_maps[count]),cols=9,count = i)
        # print(attn_maps[count][0])

    print(list(cache.keys()))
    attn_maps = cache['Model.forward']
    print(len(attn_maps))
    print(attn_maps[0].shape)
    for i in random.sample(range(int(len(attn_maps))), 3):
        count = i
        # visualize_head(np.array(attn_maps[count])[0],count = i)
        # print(attn_maps[count][0])
    # 保存预测结果
    df = pd.DataFrame({
    ...
    df.to_csv('/home/xycui/project/af3_binding/results/' + model_name + '/'+epoch_name+'.csv', index=False)
